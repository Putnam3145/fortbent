local function getGristValue(unit)
    ...
    for k,v in ipairs(unit.status.current_soul.skills) do
        local add_func=skill_value[df.job_skill.attrs[v.id].type] or (df.job_skill[v.id]=='SITUATIONAL_AWARENESS' and function(v.rating) return rating^2 end)
...
local function gristTorrent()
    ...
    local torrent_amount=#civ.hist_figures*10
    grist.ints[1]=grist.ints[1]+torrent_amount --each hist figure gets ten grist a day. Note also that I think this means that your population will increase the grist you get linearly, so that's some ramp-up.
...
require('repeat-util').scheduleUnlessAlreadyScheduled('GristTorrent',1,'days',gristTorrent)